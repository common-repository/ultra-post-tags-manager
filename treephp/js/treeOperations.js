...
function TreeOperations()
{
	...
	this.inputText = "<input   type='text' id='inputText' style='width:200px;' >";
...
//**********************************************************
	...
	{    // Menu de yeni eleman ekle se�enegi tiklandiginda ilk bura �agrilir 
		...
		$('#inputText').focus();
		if (!folder){//only allow autocomplete file
				var getdatatags=window.parent.func_getdatatags()
				$("#inputText").autocomplete(getdatatags, {
					//mustMatch: true,
					multiple: true,
					autoFill: true,
					multipleSeparator: ", "
...
*******************************************************/
	...
	this.updateElementNameReq = function(folder)
	{
		if ( treeOps.isTreeBusy() == true ||  
			 treeOps.trGetSelectedWithAlert() == null
			) 
		...
		$('span:first', treeOps.trGetSelected()).replaceWith("<input type='text' id='inputText' style='width:200px;' value='"+elementName+"'/>");	
		$('#inputText').focus();
		if (!folder){//only allow autocomplete file
			var getdatatags=window.parent.func_getdatatags()
			$("#inputText").autocomplete(getdatatags, {
