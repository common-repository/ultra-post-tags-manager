...
$(document).ready(function() {
	...
	$('#myMenu1 .edit').click(function() {  treeOps.updateElementNameReq(true); });
	$('#myMenu2 .edit').click(function() {  treeOps.updateElementNameReq(); });
	...
	simpleTree = $('.simpleTree').simpleTree({
		...
		afterDblClick:function(node){
			...
			if ($("ul li:nth-child(1)", node).length  <=0) {
				//file
				var datatext= $("#suggestTags", top.document).val() +$('span:first',node).text() + ", ";
				 $("#suggestTags", top.document).val(datatext);
				 window.parent.func_treeview_to_textbox();
			}
